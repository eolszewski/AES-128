...
public class aes {
	...
	public static void main(String[] args) {
		...
		// Build sBoxMatrix
		String sBoxMatrix[][] = sBoxMatrix(false);
		
		// Build array of round keys
		ArrayList<String[]> roundKeys = makeRoundKeys(sBoxMatrix(true), RconMatrix, key);
			
		// Create a state array
		String state[][] = new String[4][4];
		for(int i = 0; i < 4; i++)
			for(int j = 0; j < 4; j++)
				state[i][j] = input.substring(8 * i + 2 * j, 8 * i + 2 * j + 2);
		
		// Rounds
		for(int roundNumber = 0; roundNumber < 11; roundNumber++) {
			if(encrypt)
				state = encrypt(state, roundKeys, roundNumber, sBoxMatrix, encrypt);
			else
				state = decrypt(state, roundKeys, roundNumber, sBoxMatrix, encrypt);
		}
		for(int i = 0; i < 4; i++)
		{
			for(int j = 0; j < 4; j++)
